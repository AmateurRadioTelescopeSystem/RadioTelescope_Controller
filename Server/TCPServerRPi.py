...
class TCPServerRpi(object):
    ...
    def createSocket(self):
        host = self.cfgD.getHost()  # Get the TCP connection host
        port = self.cfgD.getPort()  # Get the TCP connection port
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Create a socket
        sock.bind((host, int(10003)))  # Bind to the socket
